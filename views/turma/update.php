...
<script>
    ...
    function preencheTabelaHorário(horario) { //Pega os dados de horários da turma e preenche a tabela
        horario.forEach(function(val) {
            ...
            $("#td"+val.semana+val.periodo).append("<hidden id='hidden"+val.semana+val.periodo+"' dia='"+val.semana+"' periodo='"+val.periodo+"' sala='"+val.sala+"' disciplina='"+val.disciplina+"'></hidden>");
            $("#td"+val.semana+val.periodo).append("<a id='linkremove"+val.semana+val.periodo+"' id_dia='"+val.semana+"' id_periodo='"+val.periodo+"' href='#' class='pull-right text-danger' style='margin-right: 2px;' title='Remover'> <span class='glyphicon glyphicon-trash'></span> </a>");
        });
        return true;
    }
